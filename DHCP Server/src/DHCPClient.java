...
public class DHCPClient {
	...
	public void init() {
		...
		while (state == DHCPClientState.INIT){
			if (this.tt != 0){				
				try{
					ErrorPrinter.print("No valid offer received, waiting " + this.tt + " seconds before retrying.");
	...
	public void select(){
		...
		System.out.println("Selecting best offer");
	...
	public void request(){
		...
		while (this.state == DHCPClientState.REQUESTING){
			if (timeout != 0){				
				try{
					System.out.println("No answer received, waiting: " + timeout + " seconds before retrying");
	...
	public void renew(){
		...
		while (this.state == DHCPClientState.RENEWING){
			...
			if (!received) {
				...
				// lease.
				if (this.rebindingTime.isBefore(LocalDateTime.now().plusSeconds(timeout))){
					this.rebind();
				}
			}
		}
